...
import { gsap, TweenMax, Power0, Power1, Power2, Power4, Elastic } from "gsap";
...
export function weatherCard(){
	...
	function init()
	{
		...
		{
			clouds[j].offset = Math.random() * sizes.card.width;
			drawCloud(clouds[j], j);
		}
	...
	function onResize()
	{
		...
		sizes.card.offset.top = card.offset().top;
		sizes.card.offset.left = card.offset().left;
	...
	function makeRain()
	{
		...
		// Draw the line
		var line = rainHolder['innerRainHolder' + (3 - Math.floor(lineWidth))].path('M0,0 0,' + lineLength).attr({
			fill: 'none',
	...
	function makeSplash(x, type)
	{
		...
		var xOffset = (sizes.container.width - sizes.card.width)
